...
def correct_kcBert(model, tokenizer, test_data, args, length_limit = None):
    ...
    for i in range(0, len(test_data), args.eval_batch_size):
        ...
        src_token_ids = [tokenizer.encode("[CLS] " + text + " [SEP]") for text in batch]
...
def train(model, tokenizer, train_data, valid_data, args):
    ...
    train_dataloader = DataLoader(train_dataset, sampler=RandomSampler(train_dataset),
                                  ...
                                  collate_fn=lambda x: collate_fn_kcBert(x, tokenizer, args.max_seq_length, add_special_token = True))
...
def main():
    ...
    model = ElectraForMaskedLM.from_pretrained("monologg/koelectra-base-v2-discriminator").to(args.device)
    tokenizer = ElectraTokenizer.from_pretrained("monologg/koelectra-base-v2-discriminator")
