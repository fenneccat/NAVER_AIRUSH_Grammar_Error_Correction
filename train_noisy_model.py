...
from dataset import TextDataset, collate_fn, collate_fn_noisy
...
def train(model, tokenizer, train_data, valid_data, args, eos=False):
    ...
    train_dataloader = DataLoader(train_dataset, sampler=RandomSampler(train_dataset),
                                  ...
                                  collate_fn=lambda x: collate_fn_noisy(x, tokenizer, args.max_seq_length, eos=eos))
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_dataloader:
            ...
            if step % args.eval_interval == 0:
                ...
                val_em = em(prediction, valid_noisy)
                cnt = 0
                for noisy, pred, clean in zip(valid_noisy, prediction, valid_clean):
                    print(f'[{clean}], [{pred}], [{noisy}]')
                    # 10개만 출력하기
                    cnt += 1
                    if cnt == 20:
                        break
                val_gleu = gleu(prediction, valid_noisy)
...
def main():
    ...
    if args.mode == "train" or args.mode == "pretrain" or args.mode == "semi-train":
        ...
        if args.mode == "semi-train":
            ...
            tokenizer = CharTokenizer.from_strings(train_sents, args.vocab_size)
            bind_nsml(model, tokenizer, args,eos=eos_setting)
            print("# of tokens: ", len(tokenizer.vocab))



        ## to load pretrained model
        #nsml.load(checkpoint='best', session='t0005/rush1-2/36')
