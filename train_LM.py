...
from dataset import TextDataset, collate_fn, collate_fn_MLM, collate_fn_LM
...
def train(model, tokenizer, train_data, valid_data, args, eos=False):
    ...
    train_dataloader = DataLoader(train_dataset, sampler=RandomSampler(train_dataset),
                                  ...
                                  collate_fn=lambda x: collate_fn_LM(x, tokenizer, args.max_seq_length, eos=eos, tokenizer_type=args.tokenizer))
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_dataloader:
            ...
            if step % args.eval_interval == 0:
                ...
                prediction = correct(model, tokenizer, valid_clean, args, eos=eos, length_limit=0.1)
                val_em = em(prediction, valid_clean)
                cnt = 0
                # print("zero index check", valid_masked[0], prediction[0], valid_clean[0])
                for noisy, pred, clean in zip(valid_clean, prediction, valid_clean):
...
def main():
    ...
    if args.mode == "train":
        ...
        train_data, valid_data = pairs+noisy_sents_labeled[:-args.num_val_data]+pairs_labeled[:-args.num_val_data], pairs_labeled[-args.num_val_data:]
