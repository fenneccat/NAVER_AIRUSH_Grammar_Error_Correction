...
from dataset import TextDataset, collate_fn_bert
...
def correct(model, tokenizer, test_data, args, eos=False, length_limit = None):
    ...
    for i in range(0, len(test_data), args.eval_batch_size):
        ...
        prediction.extend([''.join([tokenizer.i2c[x] for tok in x if tok >= 4]).strip() for x in tgt_token_ids])
    return prediction
...
def correct_kcBert(model, tokenizer, test_data, args, length_limit = None):
    ...
    for i in range(0, len(test_data), args.eval_batch_size):
        ...
        src_token_ids = [[2]+tokenizer(x)+[3] for x in batch]  # add eos token
        ...
        prediction.extend([''.join([tokenizer.i2c[tok] for tok in x if tok >= 4]).strip() for x in batch_predict_token_ids])
...
def train(model, tokenizer, train_data, valid_data, args):
    ...
    train_dataloader = DataLoader(train_dataset, sampler=RandomSampler(train_dataset),
                                  ...
                                  collate_fn=lambda x: collate_fn_bert(x, tokenizer, args.max_seq_length))
